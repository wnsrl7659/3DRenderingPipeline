...
File Name: HierarchyTopDownAABB.h
...
class HierarchyTopDownAABB : public BVolumeHierarchy
{
public:
  ...
  HierarchyTopDownAABB(std::list<Object*>& _pObjectList)
  {
    int currentLevel = 0;
    ...
    TopDownAABBAux(pointCloud, &(this->node), currentLevel);
  }

  ~HierarchyTopDownAABB()
  {
    ClearAllNodes(&(this->node));
  }

  void TopDownAABBAux(const std::vector<glm::vec3>& pointCloud, Node* pNode, int _currentLevel)
  {
    ...
    VolumeType* pBVol = new VolumeType(pointCloud);
    ...
    //if (pointCloudLeft.size() >= MAX_HARD_CUTOFF_VERTICES)
      TopDownAABBAux(pointCloudLeft, pLeft, _currentLevel + 1);

    //if (pointCloudRight.size() >= MAX_HARD_CUTOFF_VERTICES)
      TopDownAABBAux(pointCloudRight, pRight, _currentLevel + 1);
